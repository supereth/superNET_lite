...
var NRS = (function (NRS, $, undefined) {
	...
	/*NRS.setupClipboardFunctionality = function() {
	    var elements = "#asset_id_dropdown .dropdown-menu a, #account_id_dropdown .dropdown-menu a, .coinaddr .dropdown-menu a";
		...
		if (NRS.inApp) {
			$el.on("click", function() {
				parent.postMessage({
					"type": "copy",
					"text": NRS.getClipboardText($(this).data("type"), $(this))
				}, "*");
			...
			clipboard.on("dataRequested", function(client, args) {
			    client.setText(NRS.getClipboardText($(this).data("type"), $(this)));
			});
			...
			clipboard.on("noflash", function (client, args) {
			    NRS.isCopyFeature = false;
			    $("#account_id_dropdown .dropdown-menu, #asset_id_dropdown .dropdown-menu, .coinaddr .dropdown-menu a").remove();
			    $(".coinaddr").removeClass("dropdown");
			    $("#account_id_dropdown, #asset_id.coinaddr, .coinaddr").data("toggle", "");
			...
			clipboard.on("wrongflash", function (client, args) {
			    ...
			    $("#account_id_dropdown .dropdown-menu, #asset_id_dropdown .dropdown-menu, .coinaddr .dropdown-menu a").remove();
			    $(".coinaddr").removeClass("dropdown");
			    $("#account_id_dropdown, #asset_id, .coinaddr").data("toggle", "");
				$.growl($.t("error_clipboard_copy_wrongflash"));
			});
		}
	}
    */
	NRS.getClipboardText = function(type,element) {
		switch (type) {
			case "account_rs":
			...
			case "asset_link":
				...
				break;
		    case "coin_addr":
		        return element.parents('.coinaddr').children('h4').text();
		        break;
			default:
				return "";
				break;
