...
var NRS = (function(NRS, $, undefined) {
	...
	NRS.getState = function(callback) {
		NRS.sendRequest("getBlockchainStatus", function(response) {
			if (response.errorCode) {
				...
				if (firstTime) {
				    $("#nrs_version").html(NRS.spnliteversion).removeClass("loading_dots");
	...
	NRS.updateBlockchainDownloadProgress = function() {
		...
		if (!NRS.serverConnect || !NRS.peerConnect) {
			...
			$("#snLoader #snLoaderTxt").hide();
			$("#snLoader #snLoaderBar").hide();
			$("#snLoader .db_halted").show();
		} else {
			$("#downloading_blockchain .db_halted").hide();
			$("#downloading_blockchain .db_active").show();

			$("#snLoader .db_halted").hide();
			$("#snLoader #snLoaderTxt").show();
			$("#snLoader #snLoaderBar").show();
			...
			if (NRS.state.lastBlockchainFeederHeight && NRS.state.numberOfBlocks <= NRS.state.lastBlockchainFeederHeight) {
			    ...
			    /* Update SN loader */
			    $("#snLoaderTxt .blocks_left").html($.t("blocks_left", { "numBlocks": blocksLeft }));
				$("#snLoaderBar span").css('width', percentageTotal + "%");
			    /* End SN loader */
